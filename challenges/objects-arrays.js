...
*/
function dinosaur(name, diet, weight, dinoLength, period) {
  this.name = name;
  this.diet = diet;
  this.weight = weight;
...
console.log(stegosaurus.dinoLength);
...
const universities = graduates.map(graduate => graduate.university).sort((a, b) => (a < b ? -1 : 1));
...
*/
const displayNames = [];
zooAnimals.forEach(({ animal_name, scientific_name }) => displayNames.push(`${animal_name} ${scientific_name}`));
...
const lowCaseAnimalNames = zooAnimals.map(({ animal_name }) => animal_name.toLocaleLowerCase());
...
*/
const populationTotal = zooAnimals.reduce((accumulator, { population }) => accumulator + population, 0);
